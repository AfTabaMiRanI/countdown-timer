...
console.log(chalk.yellowBright(`\t *****==== COUNTDOWN TIMER ====***** \n`));
    ...
    [
        {
            name: "userTimer",
            type: "number",
            message: chalk.greenBright("please enter the amount of second:"),
            validate: (input) => {
                if(isNaN(input)){
                    return chalk.blue.bold("please Enter a valid number")
                }
                else if(input > 60){
                    return chalk.blue.bold("Seconds must be in 60")
                }
                else{
                    return true
                }
...
let input = result.userTimer
 ...
 function start_Time(seconds : number){
...
setInterval(() => {
    ...
    if (timeDif >= 0) {
        ...
        console.log(chalk.blueBright.bold("Time's up! :"));
        process.exit()
    }
}, 1000);
}
